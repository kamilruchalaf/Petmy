...
Petmy is a two-track product workspace: the production-oriented web app lives in `web/`, while the Kotlin Compose Multiplatform mobile experiment lives in `multiplatform/`. Keep this root guide focused on cross-repo rules; load the nested guide before editing either app.

## Hard Rules

- Write Markdown files in English.
- Use `web/AGENTS.md` for Astro, React, Supabase, Cloudflare, npm scripts, and web auth conventions.
- Use `multiplatform/AGENTS.md` for Kotlin, Compose Multiplatform, Gradle, Android, desktop, and iOS wrapper conventions.
...
- `web/` is the web and backend implementation. Its local contract is `@web/AGENTS.md`; deeper setup details live in `@web/README.md`.
- `multiplatform/` is the Kotlin Compose Multiplatform implementation. Its local contract is `@multiplatform/AGENTS.md`; scaffold notes live in `@multiplatform/README.md`.
- `context/foundation/` contains planning handoffs: `@context/foundation/prd.md`, `@context/foundation/tech-stack.md`, and `@context/foundation/tech-stack-mobile.md`.
- `context/changes/bootstrap-verification/` stores bootstrap verification notes. Keep mobile verification files suffixed with `-mobile`.
- `.agents/skills/` contains local workflow skills. Update skill instructions there, not in this root onboarding file.

## Repo Workflow

Run app-specific checks from the app directory, not from the repo root. There is no root package manager or root Gradle wrapper; `npm` belongs in `web/`, and `./gradlew` belongs in `multiplatform/`.

Recent history uses short imperative commit subjects such as `Add web app scaffold` and `Add 10x agent context tools`. Keep new commit subjects concise and scoped to the changed area.

## Agent Context

Codex and similar tools load the closest `AGENTS.md`. If you work under `web/` or `multiplatform/`, the nested file is authoritative for local commands and conventions; this root file only supplies repo-wide boundaries and naming rules.
